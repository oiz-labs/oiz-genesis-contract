...
{
    /*----------------- constants -----------------*/
    string private constant NAME = "OIZ Governance Token";
    string private constant SYMBOL = "govOIZ";
    ...
    /*----------------- external functions -----------------*/
    /**
     * @dev Sync the account's govOIZ amount to the actual OIZ value of the StakingCredit he holds
    ...
    /**
     * @dev Batch sync the account's govOIZ amount to the actual OIZ value of the StakingCredit he holds
    ...
    /**
     * @dev delegate govOIZ votes to delegatee
    ...
    function _sync(address stakeCredit, address account) internal {
        uint256 latestOIZAmount = IStakeCredit(stakeCredit).getPooledOIZ(account);
        uint256 _mintedAmount = mintedMap[stakeCredit][account];

        if (_mintedAmount < latestOIZAmount) {
            uint256 _needMint = latestOIZAmount - _mintedAmount;
            mintedMap[stakeCredit][account] = latestOIZAmount;
            _mint(account, _needMint);
        } else if (_mintedAmount > latestOIZAmount) {
            uint256 _needBurn = _mintedAmount - latestOIZAmount;
            mintedMap[stakeCredit][account] = latestOIZAmount;
